...
class MyGUI(tk.Tk):
    ...
    def __init__(self,img,lablogo,polimi_logo,num_leds,pipeline, resetbutton_publisher, handdetection_listener, rosnode):
        super().__init__()
        ...
        self.reset_buttons={}
        self.reset_button_pub = resetbutton_publisher #rospy.Publisher("/guiresetbutton",String,queue_size=1)
        self.listener = handdetection_listener #rospy.Subscriber('/handdetection/flag', Bool, self.sub_callback, queue_size=1)
    ...
    #         self.rosnode.get_logger().info.loginfo(" ********************** Safety Action is Activated ********************.")
        ...
        if frame is not None:
            ...
            # Hand detection
                if hand_presence and self.recovery_from_safety_flag:
                    msg = Bool()
                    msg.data = True 
                    self.rosnode.pub_handpresence_.publish(msg)
             
                    self.rosnode.get_logger().info("Hand is in the dangerous Zone!!!!!!!")
...
class GUIROS(Node):
    def __init__(self):
        ...
        self.sub_message_from_heat = self.create_subscription(String,'/message_to_gui', self.sub_message_cb, 1)
        self.recovery_from_safety_sub = self.create_subscription(Bool, '/recovery_from_safety',self.sub_recover_from_safety_cb, 1)
        self.reset_button_pub = self.create_publisher(String, "/guiresetbutton",1)
        self.handdetection_listener = self.create_subscription(Bool,'/handdetection/flag', self.sub_callback, 1)

        self.get_logger().info("Gui node has been started.")

    def sub_message_cb(self, msg:String):
        txt = msg.data
        global mygui
        mygui.insert_text_and_scroll(f"{txt}\n\n") 

    def sub_recover_from_safety_cb(self,msg:Bool):
        global mygui
    ...
    def sub_callback(self,data):
        handpresense = data.data
        if handpresense==True:
            self.get_logger().info.loginfo(" ********************** Safety Action is Activated ********************.")
            global mygui
            mygui.pcb_status_indicator(lednumber=1)
...
#################################
    ...
    mygui = MyGUI(image,lablogo=image_logo,polimi_logo = image_polimilogo ,num_leds=7,pipeline=None,
                  resetbutton_publisher=rosnode.reset_button_pub,
                  handdetection_listener=rosnode.handdetection_listener,
                  rosnode=rosnode)
