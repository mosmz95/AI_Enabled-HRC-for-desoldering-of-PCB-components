...
from custom_interfaces.msg import Componentdata, Llmfeedback,Safetycheck
...
class ComponentPublisherNode(Node):
    ...
    def sub_comingframe_cb(self,msg:Safetycheck):
        ...
        if self.startframe_topcamera == True:
            self.incomming_frame = self.imagebridge.imgmsg_to_cv2(msg.annotated_image, desired_encoding='bgr8') 
            # print(msg.annotated_image)
            self.handpresence_ = msg.hand_presence.data# for conversion of cv images to ros2 images
            # print("hiii")
            if self.handpresence_ == True:
                self.get_logger().warn("Your hand is in the dangerous zone.")
                self.displayedtxt = "Your hand is in the dangerous zone."
        ...
        if component_or_class == "ComponentClass" :
            ...
            if component_class >= 0 and component_class < 16:
                ...
                if self.wrist_camera_image is not None:
                    ...
                    # self.component_id = component_class
            else:
...
class ComponentGUI:
    def __init__(self, ros_node,file_path):
        ...
        self.completed_button.place(relx=0.4, rely=0.4, anchor='nw')
